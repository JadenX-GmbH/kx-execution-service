...
public class GigControllerTest extends BaseIT {
    ...
    @Test
    @Sql({"/data/datasetData.sql", "/data/gigData.sql"})
    public void getDatasetByGig_success() {
        var headers = headers();
        headers.setBearerAuth(JWT_TOKEN);
        final HttpEntity<String> request = new HttpEntity<>(null, headers);
        final ResponseEntity<PaginatedResponse<?>> response = restTemplate.exchange(
            "/api/gigs/1400/datasets?page=0&size=1", HttpMethod.GET ,
    ...
    public void getGig_success() {
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(1400, response.getBody().getId());
    }
    ...
    public void createGig_missingField() {
        ...
        assertEquals("id", response.getBody().getFieldErrors().get(0).getField());
    }
    ...
    public void updateGig_success() {
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(1400L, gigRepository.findById(1400L).get().getId());
    }
